...
public class FetchData {
	...
	public static void main(String[] args) {
		 ...
		 * addAnnotatedClass(Laptop.class); SessionFactory sf =
		 * con.buildSessionFactory(); Session session = sf.openSession(); l =
		 * session.get(Student.class, 101); // check for first level cache
		 * session.get(Student.class, 101); System.out.println(l); // commit only if tx
		 * still hasn't been committed yet (by hibernate)
		 * if(session.getTransaction().getStatus().equals(TransactionStatus.ACTIVE))
		 * session.getTransaction().commit(); session.close();
		 * 
		 * // second session
		 * 
		 * Session session1 = sf.openSession(); l = session1.get(Student.class, 101);
		 * System.out.println(l); // commit only if tx still hasn't been committed yet
		 * (by hibernate)
		 ...
		 * session.createQuery("from Student where rno = 101"); q.setCacheable(true); //
		 * make query result cacheable or future use
		 * 
		 * Student l = (Student)q.uniqueResult(); System.out.println(l);
		 * 
		 * session.getTransaction().commit(); session.close();
		 * 
		 * 
		 * Session session1 = sf.openSession(); session1.beginTransaction(); Query q2 =
		 * session1.createQuery("from Student where rno = 101"); q2.setCacheable(true);
		 * // make query result cacheable or future use Student l1 =
		 * (Student)q2.uniqueResult(); System.out.println(l1);
		 * session1.getTransaction().commit(); session1.close();
		 */		
